...
server <- function(input, output) {
    ...
    observeEvent(
        input$apply_button,
        {
            if(!(input$effect_size_input == 0)&
                ...
                !(input$precision== 0)) {
               users_data$data <- tibble(rr = input$effect_size_input,
                      prev_outc = input$prevalence_input,
    ...
    output$plot <- renderPlot({
        p <- ggplot() +
            geom_point(data = comparisons, aes(prev_outc, rr))
        if (!is.null(users_data$data)) {
            p <- p + geom_point(data = users_data$data, aes(prev_outc, rr), shape = 1, fill = "white", size = 5)
            p <- p + geom_errorbar(data = users_data$data, aes(x = prev_outc, ymax = error_max, ymin = error_min))
         }
        p <- p + plot_theme
    ...
    output$table <- renderDT(datatable(comparisons,
